...
void generar_archivo_de_alumnos(FILE *pf_lectura,FILE *pf_escritura,FILE *pf_errores,t_fecha *fecha_proceso,const char *path_idx)
{
    ...
    //contador del número de registro del índice
    unsigned i=0;
                 ...
                 &registro.f_ingreso.d,&registro.f_ingreso.m,&registro.f_ingreso.a,registro.carrera,&registro.materias_aprob,cadena)!=EOF)
    {
        ...
        if(*cadena!='\0')
        {
            ...
            if(*aux)
            {
                sscanf(cadena,"%2d/%2d/%4d",&registro.f_ult_mat_aprob.d,&registro.f_ult_mat_aprob.m,&registro.f_ult_mat_aprob.a);
        ...
        //grabo el registro en el archivo de alumnos
        fwrite(&registro,sizeof(t_alumno),1,pf_escritura);

        //cargo el registro de índice e incremento el contador de número de registro
        reg_indice.dni=registro.dni;
        reg_indice.nro_reg=i;
        i++;

        //inserto el registro de índice en el índice
        if(!ind_insertar(&indice,&reg_indice))
            printf("\n\n ADVERTENCIA:\t Uno de los registros no se ha podido insertar en el índice \n\n\a\a");

        //vuelvo al comienzo del bucle y leo el siguiente registro
    }

    //finalizado la lectura de registros, procedo a exportar el índice a un archivo
    if(ind_grabar(&indice,path_idx))
        printf("\n\n El archivo del ÍNDICE de ALUMNOS fue generado exitosamente...");
    else
        printf("\n\n ADVERTENCIA:\t El índice no se ha podido grabar en un archivo \n\n\a\a");

    //finalmente, vacío el índice ya que no lo necesito más
    ind_vaciar(&indice);
